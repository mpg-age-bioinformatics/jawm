...
""",
    ...
    # Kubernetes logs merge stdout+stderr -> stderr lines may be in output, not .error
    if (procA.manager or "").lower() == "kubernetes":
        assert "A STDERR 4" in outA, "❌ tail_conc_1 stderr missing from combined logs"
        assert "B STDERR 8" in outB, "❌ tail_conc_2 stderr missing from combined logs"
    else:
...
print("\n>>> Test 20: Parallelism True vs False (time-window overlap, backend-agnostic)")

try:
    logs_dir = "logs_test_parallel"
    os.makedirs(logs_dir, exist_ok=True)

    def parse_start_end(out: str):
        m1 = re.search(r"^START\s+(\d+)\s*$", out, flags=re.M)
        m2 = re.search(r"^END\s+(\d+)\s*$", out, flags=re.M)
        assert m1 and m2, f"Missing START/END markers in output:\n{out}"
        return int(m1.group(1)), int(m2.group(1))

    sleep_sec = 8
    tol = 2  # seconds tolerance for scheduler/log jitter

    script_timed = f"""#!/bin/bash
set -euo pipefail
echo "START $(date +%s)"
sleep {sleep_sec}
echo "END $(date +%s)"
"""
    ...
    # ---------------- A) parallel=True should overlap ----------------
    pA = Process(name="parallel_true_A", script=script_timed, logs_directory=logs_dir)
    pB = Process(name="parallel_true_B", script=script_timed, logs_directory=logs_dir)
    ...
    assert pA.get_exitcode().startswith("0"), f"❌ parallel=True: A failed\n{pA.get_output()}\n{pA.get_error()}"
    assert pB.get_exitcode().startswith("0"), f"❌ parallel=True: B failed\n{pB.get_output()}\n{pB.get_error()}"

    sA, eA = parse_start_end(pA.get_output())
    sB, eB = parse_start_end(pB.get_output())

    latest_start = max(sA, sB)
    earliest_end = min(eA, eB)

    # overlap: latest start < earliest end (with tolerance)
    assert latest_start < (earliest_end - tol), (
        f"❌ Expected overlap but did not see it.\n"
        f"A: start={sA} end={eA}\n"
        f"B: start={sB} end={eB}\n"
        f"latest_start={latest_start} earliest_end={earliest_end} tol={tol}\n"
        f"elapsed_parallel={elapsed_parallel:.1f}s\n"
    )

    # ---------------- B) parallel=False should NOT overlap ----------------
    pC = Process(name="parallel_false_C", script=script_timed, logs_directory=logs_dir, parallel=False)
    pD = Process(name="parallel_false_D", script=script_timed, logs_directory=logs_dir, parallel=False)
    ...
    assert pC.get_exitcode().startswith("0"), f"❌ parallel=False: C failed\n{pC.get_output()}\n{pC.get_error()}"
    assert pD.get_exitcode().startswith("0"), f"❌ parallel=False: D failed\n{pD.get_output()}\n{pD.get_error()}"

    sC, eC = parse_start_end(pC.get_output())
    sD, eD = parse_start_end(pD.get_output())

    # serial: D should start after C ends (with tolerance)
    assert sD >= (eC - tol), (
        f"❌ Expected serial ordering but saw overlap.\n"
        f"C: start={sC} end={eC}\n"
        f"D: start={sD} end={eD}\n"
        f"tol={tol}\n"
        f"elapsed_serial={elapsed_serial:.1f}s\n"
    )
