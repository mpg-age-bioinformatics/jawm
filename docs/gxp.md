...
Use versioned workflows and [pin remote modules](module/run.md#pinning-to-a-ref) to an immutable commit or an approved release:
```
...
When a workflow is run through the [`jawm` CLI](cli/jawm.md), its transcript contains an environment report, for example:
...
For local and Slurm execution, [`environment`](process/parameters.md#environment) selects the container runtime: `environment="docker"` uses Docker, while `environment="apptainer"` uses Apptainer (`"singularity"` is also accepted). The [`container`](process/parameters.md#container) argument identifies the image to run, such as `container="mpgagebioinformatics/fastqc:0.11.9"` for Docker or `container="/containers/fastqc-0.11.9.sif"` for Apptainer. Both arguments must be set to enable container execution: `environment` defaults to `"local"`, and requesting a container runtime without supplying `container` makes jawm fall back to local execution.

jawm resolves the process script and constructs the corresponding `docker run` or `apptainer exec` command, including configured mounts and environment variables, to execute that script inside the selected image. The generated script and launch command are recorded in the process log folder. The [`manager`](process/parameters.md#manager) argument separately selects where the job is launched. With `manager="kubernetes"`, `container` specifies the Kubernetes Job's image directly; `environment` does not select a Docker or Apptainer wrapper for that job, and the generated Job manifest records the container configuration.
...
[`validation="strict"`](process/parameters.md#validation) can detect common process configuration problems before execution, and [`jawm-test`](cli/jawm-test.md) can compare selected output hashes with approved regression references across configured environments. These are useful verification controls, but they do not replace documented computerized-system validation, change control, user training, or review procedures required by the organization.
